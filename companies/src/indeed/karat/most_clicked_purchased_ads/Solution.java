package indeed.karat.most_clicked_purchased_ads;
...
class Solution {
    ...
    public void calculateAdDetailsMatrix(String[][] adClicks, String[][] allUserIPs, Set<String> completedPurchaseIds) {

        Map<String, IPHolder> IPHolders = new HashMap<>();
        for (String[] userIP : allUserIPs) {
            IPHolders.put(userIP[1].trim(), new IPHolder(userIP[1].trim(), userIP[0].trim()));
        }
        Map<String, List<IPHolder>> adClickDetails = new HashMap<>();
        for (String[] adClick : adClicks) {
            adClickDetails.putIfAbsent(adClick[2], new ArrayList<>());
            adClickDetails.get(adClick[2]).add(IPHolders.get(adClick[0]));
        }


        //compute:
        for (String adText : adClickDetails.keySet()) {
            List<IPHolder> clickedIPHolders = adClickDetails.get(adText);
            int clickedAd = clickedIPHolders.size();
            int purchased = 0;
            for(IPHolder holder: clickedIPHolders) {
                if(completedPurchaseIds.contains(holder.userID)) {
                    purchased++;
                }
            }

            System.out.printf("%d of %d %s%n", purchased, clickedAd, adText);
        }
    }

    private static class IPHolder {
        private String IP;
        private String userID; // this is assgined by the company

        private IPHolder(String ip, String userID) {
            IP = ip;
            this.userID = userID;
        }
